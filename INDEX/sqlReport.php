...
CREATE UNIQUE INDEX unique_userName ON CRM_userTable (userName);
...
DROP INDEX unique_userName ON CRM_userTable;
...
#分析概括
#选择加索引的语句都是通过mysqlsla工具分析，按照出现率和join联表查询进行分析，并也综合考虑了数据的增加速度，得面下面的所有语句，利用SQL explain 分析查找哪个表下的哪个字段需要增加索引。
#下面有的时单条语句，表面上看不需要加，但是在其他join查询该字段都会经常被用到。
#例如 3、
   #LCDScreen.location_id [index](necessary)
   #我只列出了4条语句 其中 location_id 每一条语句中都需要对它查询 或 进行关联，所以给它加上索引是有很大的好处的。
   
#这次索引列出来得只是一部分，使用的数据不多。如果不出问题把索引加上后,需要把以前的slow.log日志备份并清空，让SQL Server 为我们重新收集有用的数据，之后在做一次分析。
...
3、[parse sql ]
   ...
   #get locations list
   #public function getScreenLocationList() 
   #in Screen Model
...
6、[parse sql]
   #Obtain information on the screen  
   #in Campaign Model
...
7、[parse sql]
   #    protected function getCampaignSelectedScreens()
   # in WebsocketAPI Model
...
8、[parse sql]
   #public function searchCityBaseOnFilterData($data){    
   #According to the selected filter screen and cities
   #in Campaign Model
...
9、[parse sql]
   #The screen switch machine for details
   #in Screen Model
...
11、[parse sql]
   #add location 查询主体公司指定的媒体公司有没有添加屏幕的权限
   #in Screen Model
...
15、[parse sql]
   #Query the address of the advertising (查询广告地址)
   #in Campaign Model
...
17、[parse sql]
   #get CRM_iMONRecommendedBasePrice city data
   #public function getRecommendedCity() 
   #in ImonAdmin Model
...
18、[parse sql]
   #get client media （获取客户的媒体）
   #public function getClientMedia($clientId, $type, $status = 0)
   #in Campaign Model
...
19、[parse sql]
   #Query all the provinces (in addition to the existing province)
   #public function getProvinceNotExist()
   #in Imonadmin Model
